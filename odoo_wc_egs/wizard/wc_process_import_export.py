...
class WcImportExportProcess(models.TransientModel):
    _name = 'wc.import.export.process'
    ...
    wc_instance_ids = fields.Many2many("wc.instance.egs", string="Instances")
    ...
    sync_price_with_product = fields.Boolean("Sync Product Price?",
                                             ...
                                             default=False)
    wc_coupon_ids = fields.Many2many('wc.coupons.egs', string="Coupons")
    wc_categ_ids = fields.Many2many('wc.category.egs', string="Category")
    wc_tag_ids = fields.Many2many('wc.tags.egs', string="Tags")
    wc_products_ids = fields.Many2many('wc.product.template.egs', string="Products")
    ...
    def default_get(self, fields):
        ...
        if 'default_instance_id' in self._context:
            ...
            instances = self.env['wc.instance.egs'].search([('state', '=', 'confirm')])
            res.update({'wc_instance_ids': [(6, 0, instances.ids)]})
        return res

    @api.multi
    def export_wc_coupons(self):
        wc_coupons_obj = self.env['wc.coupons.egs']
    ...
    def update_wc_categs(self):
        wc_categ_obj = self.env['wc.category.egs']
        for wc_instance in self.wc_instance_ids:
            instance_context = dict(self.env.context)
            instance_context.update({'lang': wc_instance.lang_id.code})
    ...
    def update_wc_tags(self):
        wc_tags_obj = self.env['wc.tags.egs']
        for wc_instance in self.wc_instance_ids:
            instance_context = dict(self.env.context)
            instance_context.update({'lang': wc_instance.lang_id.code})
    ...
    def export_wc_products(self):
        wc_tmpl_obj = self.env['wc.product.template.egs']
        for wc_instance in self.wc_instance_ids:
            instance_context = dict(self.env.context)
            instance_context.update({'lang': wc_instance.lang_id.code})
            ...
            if self.wc_products_ids:
                ...
                wc_products = wc_tmpl_obj.search(
                    [('wc_instance_id', '=', wc_instance.id), ('avail_in_wc', '=', False)])
            wc_products and wc_tmpl_obj.export_wc_products(wc_instance, wc_products, update_price=self.is_update_price,
                                                           update_stock=self.is_update_stock, publish=self.publish,
    ...
    def import_wc_coupons(self):
        for instance in self.wc_instance_ids:
            ...
            self.env['wc.coupons.egs'].import_coupons(instance)
        return True
    ...
    def import_wc_categs(self):
        for instance in self.wc_instance_ids:
            ...
            instance_context.update({'lang': instance.lang_id.code})
            self = self.with_context(instance_context)
            self.env['wc.category.egs'].sync_product_category(instance, self.sync_images)
        return True
    ...
    def import_wc_tags(self):
        for instance in self.wc_instance_ids:
            ...
            instance_context.update({'lang': instance.lang_id.code})
            self = self.with_context(instance_context)
            self.env['wc.tags.egs'].sync_product_tags(instance)
        return True
    ...
    def import_wc_products(self):
        for instance in self.wc_instance_ids:
            ...
            wc_products = []
            if self.wc_products_ids:
                wc_products = self.env['wc.product.template.egs'].search(
                    [('id', 'in', self.wc_products_ids.ids), ('wc_instance_id', '=', instance.id),
                     ('avail_in_wc', '=', True)])
                if not wc_products:
                    continue
            self.env['wc.product.template.egs'].import_wc_products(instance, wc_tmpl_ids=wc_products,
    ...
    def prepare_wc_product(self):
        wc_template_obj = self.env['wc.product.template.egs']
        wc_product_obj = self.env['wc.product.product.egs']
        wc_product_categ = self.env['wc.category.egs']
        wc_product_image_obj = self.env['wc.product.image.egs']
            ...
            for odoo_template in odoo_templates:
                ...
                if odoo_template.image:
                    wc_product_image_obj.create(
                        {'sequence': 0, 'wc_instance_id': wc_instance.id, 'image': odoo_template.image,
    ...
    def check_products(self, wc_templates):
        if self.env['wc.product.product.egs'].search(
    ...
    def filter_templates(self, wc_templates):
        ...
        for wc_template in wc_templates:
            if not self.env['wc.product.product.egs'].search(
