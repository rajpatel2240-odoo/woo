...
class SaleOrder(models.Model):
    ...
    wc_payment_gateway_id = fields.Many2one("wc.payment.gateway.egs", "Payment Gateway")
    ...
    order_status = fields.Many2one('wc.order.status.egs')

    @api.multi
    def cancel_so_in_wc(self):
        view = self.env.ref('odoo_wc_egs.view_wc_cancel_order_wizard')
        return {
    ...
    def search_wc_product(self, line, wc_instance):
        wc_product_obj = self.env['wc.product.product.egs']
        odoo_product_obj = self.env['product.product']
        variant_id = line.get('variation_id', '') or line.get('product_id')
        sku = line.get('sku')
    ...
    def create_or_update_wc_payment_gateway(self, wc_instance, result):
        wc_payment_gateway_obj = self.env["wc.payment.gateway.egs"]
    ...
    def import_wc_orders(self, wc_instance=False):
        ...
        for wc_instance in instances:
            ...
            for order in order_ids:
                ...
                wc_order_status = self.env['wc.order.status.egs'].search(
                ...
                for tax_line in order.get('tax_lines'):
                    ...
                    if rate_id:
                        tax_wc_job = self.env['wc.process.job.egs'].create(
                ...
                self.env['wc.order.status.egs'].process_order_autoworkflow(sale_order, wc_job)
    ...
    def auto_update_wc_order_status(self, ctx={}):
        ...
        wc_instance = wc_instance_id and wc_instance_obj.search([('id', '=', wc_instance_id), ('state', '=', 'confirm')]) or False
        if wc_instance:
