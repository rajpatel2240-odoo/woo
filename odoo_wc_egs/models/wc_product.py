...
class WcProductTemplate(models.Model):
    ...
    wc_instance_id = fields.Many2one("wc.instance.egs", "Instance", required=True, ondelete='cascade')
    product_tmpl_id = fields.Many2one("product.template", "Product Template", required=1)
    wc_categ_ids = fields.Many2many('wc.category.egs', 'wc_template_categ_rel', 'wc_template_id', 'wc_categ_id',
                                    "Categories")
    wc_tag_ids = fields.Many2many('wc.tags.egs', 'wc_template_tags_rel', 'wc_template_id', 'wc_tag_id', "Tags")
    wc_tmpl_id = fields.Char("WooCommerce Template Id")
    avail_in_wc = fields.Boolean("Available in WooCommerce")
    wc_product_ids = fields.One2many("wc.product.product.egs", "wc_template_id", "Products")
    wc_gallery_image_ids = fields.One2many("wc.product.image.egs", "wc_product_tmpl_id", "Images")
    ...
    def sync_wc_product_categ(self, wcapi, wc_instance, wc_categories, sync_images_with_product=False,
                              wc_job=False):
        wc_product_categ_obj = self.env['wc.category.egs']
        categ_ids = []
    ...
    def sync_wc_product_tags(self, wcapi, wc_instance, wc_tags):
        obj_wc_product_tags = self.env['wc.tags.egs']
        tag_ids = []
    ...
    def sync_wc_attribute_term(self, wc_instance, wc_job=False):
        obj_wc_attribute = self.env['wc.product.attribute.egs']
        obj_wc_attribute_term = self.env['wc.product.attribute.egs.term']
    ...
    def set_variant_sku(self, wc_instance, result, product_template, sync_price_with_product=False):
        ...
        wc_attribute_obj = self.env['wc.product.attribute.egs']
        wc_attribute_term_obj = self.env['wc.product.attribute.egs.term']
                ...
                try:
                    ...
                    if res_img.status_code == 200:
                        ...
                        if key in existing_gallery_img_keys:
                            ...
                            continue
                        if position == 0:
                            wc_template.product_tmpl_id.write({'image': binary_img_data})
    ...
    def import_wc_products(self, wc_instance, wc_tmpl_ids=False, update_price=False,
        ...
        if wc_instance.attribute_type == 'select':
            self.sync_wc_attribute(wc_instance, wc_job=wc_job)
        wc_product_obj = self.env['wc.product.product.egs']
        wc_product_img = self.env['wc.product.image.egs']
        product_template_obj = self.env['product.template']
        odoo_product_obj = self.env['product.product']
        wcapi = wc_instance.wc_connect()
        ...
        if categ_and_tag_imported:
            self.env['wc.category.egs'].sync_product_category(wc_instance,
                                                              sync_images_with_product=sync_images_with_product)
            self.env['wc.tags.egs'].sync_product_tags(wc_instance)
        if not results:
            return False
        for result in results:
            odoo_product = False
            ...
            for variation in result.get('variations'):
                ...
                if not is_importable:
                    wc_job.env['wc.process.job.egs.line'].create(
                ...
                if not wc_product:
                    if not wc_template:
                        ...
                        if not categ_and_tag_imported:
                            ...
                            wc_categs = []
                            for wc_category in wc_categories:
                                wc_categ = wc_category.get('id') and self.env['wc.category.egs'].search(
                        ...
                        if not categ_and_tag_imported:
                            ...
                            product_tags = []
                            for wc_tag in wc_tags:
                                product_tag = wc_tag.get('id') and self.env['wc.tags.egs'].search(
        ...
        for wc_templates in batches:
            ...
            for template in wc_templates:
                ...
                for variant in template.wc_product_ids:
                    ...
                    if var_url:
                        if template.wc_tmpl_id != variant.variant_id and variant.product_id.attribute_value_ids:
                            info.update({'image': {'id': var_url}})
                            data.get('variations').append(info)
                        elif template.wc_tmpl_id == variant.variant_id and not variant.product_id.attribute_value_ids:
                            del data['variations']
    ...
    def auto_update_product_stock(self, ctx={}):
        wc_product_tmpl_obj = self.env['wc.product.template.egs']
        wc_instance_obj = self.env['wc.instance.egs']
    ...
    def update_wc_product_stock(self, wc_instance=False, wc_products=False):
        ...
        instances = []
        if not wc_instance:
            instances = self.env['wc.instance.egs'].search(
            ...
            for wc_template in wc_templates:
                ...
                for variant in wc_template.wc_product_ids:
                    if variant.variant_id:
                        ...
                        if wc_template.wc_tmpl_id != variant.variant_id and variant.product_id.attribute_value_ids:
                            info.get('variations').append(
                                {'id': variant.variant_id, 'regular_price': str(price)})
                        elif wc_template.wc_tmpl_id == variant.variant_id and not variant.product_id.attribute_value_ids:
                            del info['variations']
        ...
        for templates in batches:
            ...
            for template in templates:
                ...
                for variant in template.wc_product_ids:
                    ...
                    var_url = ''
                    if update_image:
                        info.update(self.get_variant_image(wc_instance, variant))
                    if template.wc_tmpl_id != variant.variant_id and variant.product_id.attribute_value_ids:
                        data.get('variations').append(info)
                    elif template.wc_tmpl_id == variant.variant_id and not variant.product_id.attribute_value_ids:
                        del data['variations']
    ...
    def import_wc_stock(self, wc_instance, wc_products=[]):
        ...
        wc_invetory = self.env['stock.inventory'].create(
            {'name': 'WCIA/',
             'wc_instance_id': wc_instance.id,
             'location_id': wc_instance.warehouse_id.lot_stock_id.id, 'filter': 'partial'})
        batches = []
            ...
            for product in products:
                ...
                stock_inventory_line = stock_inventory_line_obj.create(
                    {"inventory_id": wc_invetory.id, "product_id": wc_product.product_id.id,
                     "location_id": wc_invetory.location_id.id, "product_qty": product.get('stock_quantity')})
...
class WcProductProduct(models.Model):
    ...
    wc_template_id = fields.Many2one("wc.product.template.egs", "WooCommerce Template", required=1, ondelete="cascade")
    avail_in_wc = fields.Boolean("Available in WooCommerce")
